 ...
 */
class SelectorWithPrefixWithoutTableTest extends TestCase
{
	/** @var Selector */
	private $selector;
	...
	{
		$this->selector = new Selector(null, "foo");
	}
	...
	{
		Assert::same("foo as foo_foo,bar as foo_bar2", (string)$this->selector->setSelects(["foo", ["bar" => "bar2"]]));
	}
}

\run(new SelectorWithPrefixWithoutTableTest());
