 ...
 */
class SelectorWithoutPrefixWithoutTableTest extends TestCase
{
	/** @var Selector */
	private $selector;
	...
	{
		Assert::same("foo,bar as bar2", (string)$this->selector->setSelects(["foo", ["bar" => "bar2"]]));
	}
}

\run(new SelectorWithoutPrefixWithoutTableTest());
